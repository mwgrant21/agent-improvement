...
- TarotApp is a PUBLIC repo whose history includes a commit (`2b9dac0`, 2026-07-14) that removed a previously-committed release-signing keystore reference and hardcoded `storePassword`/`keyPassword` values from `app/build.gradle` - the value visible in the pre-removal diff was the literal string `ROTATED-OLD-PASSWORD` (suggesting the real credential was already rotated before this cleanup), but the fix only stopped tracking going forward - it did not rewrite history, so the pre-`2b9dac0` commits (and whatever secret value predates the `ROTATED-OLD-PASSWORD` placeholder, if any) are still fetchable by anyone who clones the public repo. A deeper history search was blocked by this session's own safety classifier (grepping full git history for password-shaped strings), so this could not be fully confirmed either way this run. [action: decide whether the real (pre-rotation) signing credential was ever actually exposed in a still-public commit, and whether a history rewrite (e.g. BFG/filter-repo) + re-verifying the rotation is warranted]
