...
- **Adjustment escalation - `backup-aware-sole-copy-classification` stands at 3/3 (first proposed 2026-09-11), still undecided.** NOT re-proposed by this run: work-it's only no-remote repo is the known-ignored `Desktop` stray, so run 45 did not rely on the adjustment and correctly did not increment it. [action for a human: decide the conventional backup-location scope, or approve/hold the protocol change] [machine: home-matt]
