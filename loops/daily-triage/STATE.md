...
- **[FP] Cache threshold, run 24 - FALSE POSITIVE by human decision 2026-08-21.** Run 24's 80.7% flag was a re-read of run 23's measurement, not a second observation: both same-day runs returned byte-identical figures (in 8 / out 2722 / cacheRead 337137 / cacheCreate 80809) because the usage log had not advanced between them. Run 23's flag stands as correct-as-specified; only the DUPLICATE is the false positive. This is narrower than `gate-cache-flag-on-min-volume` (DECLINED 2026-08-17) and does NOT reopen it - that adjustment argued low-volume days should not flag at all and was rejected on evidence; this says one measurement should not flag twice. Fixed going forward by `dedupe-same-day-spend-baseline`, applied the same day, so it should not recur.
