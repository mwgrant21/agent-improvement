...
- **R3 COMPLETE 2026-09-11 (home-matt, human-directed) - Retrospective 5 is CLOSED.** All proposals decided and applied: P1 (prose-reliance cross-check, LOOP.md step 3), P2 (scope-step5-git-add-to-own-loop-paths + the general rule in loops/README.md), P4 (four count fields are required scalars), P5 (detect-content-duplicate-branches via `git cherry`, later hardened three times), P6 (notes.local_repo_remotes). P7 (promotion gate NOT MET - stay at L1) and P8 (duration watch-only) were report-only by design. Ledger: 42 LANDED, 2 CLOSED-MOOT, **0 OUTSTANDING** - the first time since it was created. `runs_since_retro` reset to 0 this date. [action: none] [machine: any]
- **RESOLVED 2026-09-11 - STATE.md rotation done, 59,414 -> 25,686 bytes.** Human-approved log-archivist pass. P3 proposed either raising the trigger or widening what is compressed; measurement contradicted both (the file was already 60KB, so a "55-60KB" threshold is breached on arrival, and the two sections it named were ~15% of the bytes). The weight was the Retrospective 5 block (42%) and Human Decisions (22%). The block moved verbatim to `STATE.archive-2026-09-11.md`; Human Decisions moved to `STATE.standing-decisions.md`, which is never compressed - it is append-only binding policy and the only section that grows without bound, so it is what would re-breach any threshold next. The 44-row ledger stays here in compact form because `count-reconfirmation-as-reproposal` reads those fields. Verified against the pre-compression file: nothing lost. [action: none] [machine: any]
- **MOVED TO WATCH LIST 2026-09-11 (home-matt, verified directly) - `EFIPartitionRemediation/feature/efi-diagnostic`.** Now 23 ahead / 0 behind master at `33a1030`, and **fully pushed** (`0 0` vs `origin/feature/efi-diagnostic`), so this is branch divergence, not sole-copy exposure - nothing is one disk failure from loss. It stays unmerged deliberately: the remaining checklist tasks need a managed machine, the affected hardware, or the signing certificate, and the branch's own `HANDOFF-diagnostic.md` says to fold it into the README *before* it merges. Work continued this session rather than stalling: a `codex exec review` found 4 P2s, 3 fixed in `33a1030` (signed package shipped the `tools/` disk-attaching harnesses unsigned and outside the integrity manifest; `Unknown` firmware reported READY without confirming UEFI; out-of-scope machines wrote no fleet CSV row), and the 4th is documented open at `docs/OPEN-whatif-midmigration-guidance.md`. Suite 271 passed / 3 elevation-dependent. **Recorded so the 20-commit rule stops re-escalating a branch whose status is known and accepted** - it will keep crossing the floor by design until the hardware work happens. [action: none until a managed machine is available; re-escalate only if it stops being pushed or master moves] [machine: any]
- **RESOLVED 2026-09-11 - the drifted hook was synced and verified by running it.** `~/.claude/hooks/pr-review-onstart.ps1` was 2 days behind the store and missing retrospective R4's fix, so `@($cursor.prs).Count` on a ConvertFrom-Json object returned 1 for every input - this session opened with "cursor has 1 tracked PR(s)" against an empty cursor. Store copy deployed (old one kept as `.bak-2026-09-11`), then executed for real: it now reports "cursor has 0 tracked PR(s)". A full sweep of all distributed files (5 hooks, 2 skills) shows everything else in sync, so the drift was isolated to this one file. [action: none] [machine: home-matt]
...
- **CORRECTED + RESOLVED 2026-09-11 (home-matt, verified directly) - `Miriels-publish` was never sole-copy, and now has a mirror too.** It was reported as sole-copy exposure because it has no git REMOTE. That is not the same thing: a complete `git bundle` was already sitting on a different physical disk (`D:\Backups\git-bundles\Miriels-publish-20260903-f322a86.bundle`). `git bundle verify` calls it "a complete history", it holds `refs/heads/main` at `f322a86`, and that is still the repo's exact HEAD - so the backup was complete AND current the whole time. The loop checks git remotes, and a git-remote check cannot see a bundle on another disk. Added anyway, because a bundle is a manual point-in-time snapshot while a mirror updates incrementally: `D:\Backups\git-mirrors\Miriels-publish.git`, wired as remote `mirror`. Verified real rather than assumed - the clone took 0.358s, which warranted checking: 123MB both sides (the 909MB figure was the working tree; history is 123MB), identical ref checksums, 14 commits both, `fsck` clean, and a throwaway tag pushed, confirmed present, then deleted from both sides to prove the update path. C: is disk 2 and D: is disk 3, both healthy, so this genuinely survives a single-disk failure - it is NOT off-site, so it does not cover fire or theft. [action: `git push mirror main` after future commits] [machine: home-matt]
